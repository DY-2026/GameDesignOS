...
Default `enforcement_mode` is `shadow`. Promote to `warn` or `enforce` only after representative evals show the checkpoint improves workflow quality without hiding useful domain work.
