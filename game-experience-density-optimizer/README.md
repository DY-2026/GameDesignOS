...
`game-experience-density-optimizer` 用于把留存、节奏、首局上头感和中段疲劳问题，改造成一周能跑完的 EPM 实验。

它不是泛泛的留存建议生成器。核心任务不是说多给奖励、多加反馈、多做内容，而是把体验问题拆成三个可控旋钮：事件频率、决策权重、叙事压缩，再给出可上线变体、埋点字典、看板字段、预注册判定规则和回滚条件。

## 定位

这个 skill 面向游戏策划、制作人、数据分析、独立开发者和 AI agent。它适合处理已经有原型、Demo、线上版本、测试录像、数据快照或明确体验问题的项目。

它解决的是从诊断到实验的断层：很多设计建议听起来对，但无法上线验证；很多 A/B 测试能跑数据，却不知道设计上到底改了什么。这个 skill 要让二者咬在一起。

核心原则：

- 先写边界，再写判断。
- 先定义 EPM 问题类型，再开旋钮。
- 每个实验变体只保留一个主旋钮，避免无法归因。
- 事件频率不是堆事件，决策权重不是堆选项，叙事压缩不是删剧情。
...
| 模式 | 用途 | 推荐交付物 |
| --- | --- | --- |
| `quick_epm_triage` | 快速判断体验密度问题 | EPM 诊断、三个旋钮优先级、最小改动建议 |
...
如果用户只说提升留存或体验密度，默认使用 `weekly_ab_plan`。如果用户只要很快判断问题，使用 `quick_epm_triage`。如果用户重点问埋点或看板，使用 `instrumentation_plan`。
...
game-experience-density-optimizer/
  ...
  examples/
    synthetic-survivors-first-session-epm-plan.md
  evals/
    evals.json
    rubric.yaml
    negative_cases.md
