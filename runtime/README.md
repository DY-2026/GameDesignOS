...
说明：`schema_version` 仍为 `1.0.0`，因为这次升级改变的是 runtime 版本、项目定位和授权边界，不要求迁移 workspace 结构。
...
v1.1 runtime 不提供托管服务、模型网关、API-key 管理器、自动 skill 执行、自治搜索、GUI dashboard、云同步、账号权限或游戏引擎生产 adapter。
...
- [v1.1 release note](../releases/v1.1.0.md)
